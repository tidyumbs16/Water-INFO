...
const AdminSensorsPage: React.FC = () => {
    ...
    if (sensor) {
      ...
      setNewSensor({
        ...
        last_calibration_date: sensor.last_calibration_date
          ? new Date(sensor.last_calibration_date).toISOString().split("T")[0]
          : null,
        next_calibration_date: sensor.next_calibration_date
          ? new Date(sensor.next_calibration_date).toISOString().split("T")[0]
          : null,
      });
    } else {
    ...
    async (districtId?: string) => {
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            `เซิร์ฟเวอร์ส่งการตอบกลับที่ไม่ใช่ JSON: ${res.status} ${res.statusText}`
          );
        }
        ...
        if (res.ok) {
          ...
          if (data && Array.isArray(data.data)) {
            ...
            console.warn(
              "รูปแบบข้อมูลเซนเซอร์ที่ได้รับไม่ถูกต้อง หรือเป็น Object ที่ไม่มี Array data:",
              data
            );
          ...
          console.log(
            "ตั้งค่า sensors ใน State แล้ว (จำนวน):",
            fetchedSensors.length
          );
          ...
          if (res.status === 401 || res.status === 403) {
            ...
            throw new Error(
              data.message || "ไม่ได้รับอนุญาต หรือ Token หมดอายุ"
            );
          }
          throw new Error(
            data.message || `ไม่สามารถดึงข้อมูลเซนเซอร์ได้: ${res.status}`
          );
        }
      } catch (err: any) {
  ...
  const fetchDistricts = useCallback(async () => {
    ...
    if (!token) {
      console.error(
        "ไม่พบ Token การยืนยันตัวตนสำหรับเขตประปา กรุณาเข้าสู่ระบบ"
      );
      return;
    }
    try {
      ...
      if (res.ok) {
        ...
        if (responseData && Array.isArray(responseData.data)) {
          ...
          console.error(
            "รูปแบบข้อมูลเขตประปาที่ได้รับไม่ถูกต้อง:",
            responseData
          );
        ...
        if (res.status === 401 || res.status === 403) {
          localStorage.removeItem("authToken");
          throw new Error(
            responseData.message || "ไม่ได้รับอนุญาต หรือ Token หมดอายุ"
          );
        }
        throw new Error(
          responseData.message || `ไม่สามารถดึงข้อมูลเขตประปาได้: ${res.status}`
        );
      }
    } catch (err: any) {
  ...
  const handleInputChange = (
    ...
    setNewSensor((prev: any) => ({
      ...prev,
      [name]:
        name === "value" ? (value === "" ? null : parseFloat(value)) : value,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      !newSensor.name ||
      !newSensor.district_id ||
      !newSensor.sensor_type ||
      !newSensor.status
    ) {
      setFormError(
        "โปรดกรอกข้อมูลที่จำเป็น (ชื่อ, เขตประปา, ประเภทเซนเซอร์, สถานะ)"
      );
      setFormLoading(false);
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        last_calibration_date: newSensor.last_calibration_date
          ? new Date(newSensor.last_calibration_date).toISOString()
          : null,
        next_calibration_date: newSensor.next_calibration_date
          ? new Date(newSensor.next_calibration_date).toISOString()
          : null,
        ...
        value:
          typeof newSensor.value === "string" && newSensor.value !== ""
            ? parseFloat(newSensor.value)
            : newSensor.value === null
              ? null
              : newSensor.value,
      };
      ...
      if (isEditing && currentSensorId) {
        res = await fetch(
          `${BACKEND_URL}/api/admin/sensors/${currentSensorId}`,
          {
            method: "PUT",
            headers: {
      ...
      if (res.ok) {
        ...
        if (res.status === 400) {
          ...
          setFormError(
            data.message ||
            `Serial Number '${newSensor.serial_number}' มีอยู่ในระบบแล้ว`
          );
          ...
          setFormError(
            data.message ||
            `เกิดข้อผิดพลาดในการ ${isEditing ? "อัปเดต" : "เพิ่ม"} เซนเซอร์`
          );
        }
      }
    } catch (err: any) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/admin/sensors/${sensorToDeleteId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 text-white">
        ...
        <p className="ml-3 text-lg text-slate-400">
  ...
  return (
  <div className="flex-1 p-10 bg-gradient-to-br from-sky-50 via-white to-sky-50 text-gray-800 min-h-screen">
    
    {/* Header */}
    <header className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <h1 className="text-4xl font-extrabold text-sky-700 flex items-center">
        <Database className="mr-3 w-10 h-10 text-sky-500" />
        จัดการข้อมูล Sensors
      </h1>
      <button
        onClick={() => openModal()}
        className="bg-gradient-to-r from-sky-500 to-sky-700 hover:from-sky-600 hover:to-sky-800 text-white font-semibold py-3 px-6 rounded-xl flex items-center space-x-2 shadow-md transition"
      >
    ...
    {/* Success Message */}
    {successMessage && (
      <div className="bg-green-50 border border-green-300 text-green-700 rounded-xl p-4 mb-6 flex items-center space-x-3">
    ...
    {/* Filter */}
    <section className="bg-white border border-sky-200 rounded-2xl shadow p-6 mb-8">
      <h2 className="text-2xl font-semibold text-sky-700 mb-4">
        ค้นหา/กรอง Sensor
      </h2>
      <label htmlFor="filterDistrict" className="block text-gray-700 text-sm font-medium mb-2">
        กรองตามเขตประปา:
      </label>
      <select
        id="filterDistrict"
        ...
        className="mt-1 block w-full md:w-1/2 px-4 py-3 bg-white border border-sky-200 rounded-xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-sky-400"
      >
    ...
    {/* Table */}
    <section>
      <div className="bg-white border border-sky-200 rounded-2xl overflow-hidden shadow">
        <table className="min-w-full divide-y divide-sky-100">
          <thead className="bg-sky-50">
            <tr>
              {["Serial Number","ชื่อ Sensor","เขตประปา","ประเภท Sensor","สถานะ","ค่าวัดล่าสุด","อัปเดตล่าสุด","การจัดการ"].map((head) => (
                <th key={head} className="px-6 py-4 text-left text-xs font-medium text-sky-700 uppercase tracking-wider">
                  {head}
                </th>
              ))}
            </tr>
          </thead>
          <tbody className="bg-gray-50 divide-y divide-sky-100">
            {sensors.length === 0 ? (
              <tr>
              ...
              sensors.map((sensor) => (
                <tr key={sensor.id} className="hover:bg-sky-50 transition">
                  <td className="px-6 py-4 text-sm font-medium">{sensor.serial_number}</td>
                  <td className="px-6 py-4 text-sm">{sensor.name}</td>
                  <td className="px-6 py-4 text-sm">{getDistrictName(sensor.district_id)}</td>
                  <td className="px-6 py-4 text-sm capitalize">{(sensor.sensor_type || "").replace(/_/g, " ")}</td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 inline-flex text-xs font-semibold rounded-full ${
                      sensor.status === "active" ? "bg-green-100 text-green-700" :
                      sensor.status === "inactive" ? "bg-gray-100 text-gray-700" :
                      sensor.status === "maintenance" ? "bg-sky-100 text-sky-700" :
                      sensor.status === "calibrating" ? "bg-blue-100 text-blue-700" :
                  ...
                  <td className="px-6 py-4 text-sm">{new Date(sensor.last_update).toLocaleString()}</td>
                  <td className="px-6 py-4 text-right text-sm font-medium">
                    <button onClick={() => openModal(sensor)} className="text-sky-600 hover:text-sky-800 mr-4">
                      <Edit className="w-5 h-5" />
                    </button>
                    <button onClick={() => openDeleteConfirmModal(sensor.id)} className="text-red-500 hover:text-red-700">
      ...
      {isModalOpen && (
        <div className="fixed inset-0  bg-opacity-70 flex items-center justify-center p-4 z-50 animate-fade-in">
          {/* Modal Container: Adjusted max-w to 'md' for a more compact size */}
          <div className="bg-slate-800/90 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-xl w-full max-w-md p-6 relative flex flex-col max-h-[90vh]">
            <button
              onClick={closeModal}
            ...
            <h2 className="text-3xl font-bold text-white mb-6 text-center flex-shrink-0">
            ...
            >
              ...
              <div>
                ...
                {isEditing && (
                  <p className="text-sm text-slate-400 mt-1">
                    ไม่สามารถแก้ไข Serial Number ได้
                  </p>
                )}
              </div>
              ...
              <div>
                <label
                  ...
                  className="block text-slate-300 text-sm font-medium mb-2"
                >
                  ชื่อ Sensor <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  id="name"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label
                    ...
                    className="block text-slate-300 text-sm font-medium mb-2"
                  >
                    เขตประปา <span className="text-red-400">*</span>
                  </label>
                  <select
                    id="district_id"
                    name="district_id"
                ...
                <div>
                  <label
                    ...
                    className="block text-slate-300 text-sm font-medium mb-2"
                  >
                    ประเภท Sensor <span className="text-red-400">*</span>
                  </label>
                  <select
                    id="sensor_type"
                    name="sensor_type"
              ...
              <div>
                <label
                  ...
                  className="block text-slate-300 text-sm font-medium mb-2"
                >
                  สถานะ <span className="text-red-400">*</span>
                </label>
                <select
                  id="status"
                  name="status"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={
                      newSensor.value !== null && newSensor.value !== undefined
                        ? String(newSensor.value)
                        : ""
                    }
                    onChange={handleInputChange}
                    disabled={formLoading}
              ...
              {/* Calibration Dates (Grouped) */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label
                    htmlFor="last_calibration_date"
                    className="block text-slate-300 text-sm font-medium mb-2"
                  >
            ...
            <div className="pt-4 flex-shrink-0">
              ...
              >
                ...
                <span>
                  {isEditing ? "บันทึกการเปลี่ยนแปลง" : "เพิ่ม Sensor"}
                </span>
              </button>
              <button
                type="button"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-slate-800/90 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-xl w-full max-w-sm p-8 relative text-center">
            ...
            <p className="text-slate-300 mb-6">
              คุณแน่ใจหรือไม่ที่ต้องการลบเซนเซอร์นี้?
