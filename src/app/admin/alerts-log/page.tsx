...
import { AlertSetting, AlertLog, District, ProblemReport } from '../../interfaces'; // เพิ่ม ProblemReport interface

const AlertManagementPage: React.FC = () => {
  const [alertSettings, setAlertSettings] = useState<AlertSetting[]>([]);
  const [alertsLog, setAlertsLog] = useState<AlertLog[]>([]);
  const [problemReports, setProblemReports] = useState<ProblemReport[]>([]); // State สำหรับเก็บรายงานปัญหา
  const [districts, setDistricts] = useState<District[]>([]); // สำหรับ Filter
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // *** แก้ไข Path การเรียก API ตรงนี้: จาก /alerts เป็น /alert-settings ***
  ...
  const handleDeleteSetting = async (settingId: number) => {
    if (!confirm('คุณแน่ใจหรือไม่ที่ต้องการลบการตั้งค่าการแจ้งเตือนนี้?')) {
      return;
    }
  ...
  const handleResolveAlert = async (alertId: number, isResolved: boolean) => {
    ...
    if (!confirm(`คุณแน่ใจหรือไม่ที่ต้องการเปลี่ยนสถานะการแจ้งเตือนนี้เป็น "${actionText}"?`)) {
      return;
    }
  ...
  const handleResolveProblemReport = async (reportId: string, isResolved: boolean) => {
    const actionText = isResolved ? 'แก้ไขแล้ว' : 'ยังไม่แก้ไข';
    if (!confirm(`คุณแน่ใจหรือไม่ที่ต้องการเปลี่ยนสถานะรายงานปัญหานี้เป็น "${actionText}"?`)) {
      return;
    }
              ...
              <tbody className="divide-y divide-slate-700">
                {alertsLog.map((alert) => (
                  <tr key={alert.id} className="hover:bg-slate-700/30 transition-colors duration-200">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">
                      {/* *** แก้ไขตรงนี้: เพิ่มการตรวจสอบ typeof === 'number' ก่อนเรียก toFixed() *** */}
